...
public class CornerImageView extends AppCompatImageView {
    ...
    private ColorStateList mBorderColor =
            ColorStateList.valueOf(CornerDrawable.DEFAULT_BORDER_COLOR);
    ...
    public CornerImageView(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.CornerImageView, defStyle, 0);
        ...
        float cornerRadiusOverride =
                a.getDimensionPixelSize(R.styleable.CornerImageView_riv_corner_radius, -1);

        mCornerRadii[Corner.TOP_LEFT] =
                a.getDimensionPixelSize(R.styleable.CornerImageView_riv_corner_radius_top_left, -1);
        mCornerRadii[Corner.TOP_RIGHT] =
                a.getDimensionPixelSize(R.styleable.CornerImageView_riv_corner_radius_top_right, -1);
        mCornerRadii[Corner.BOTTOM_RIGHT] =
                a.getDimensionPixelSize(R.styleable.CornerImageView_riv_corner_radius_bottom_right, -1);
        mCornerRadii[Corner.BOTTOM_LEFT] =
                a.getDimensionPixelSize(R.styleable.CornerImageView_riv_corner_radius_bottom_left, -1);
        ...
        mBorderWidth = a.getDimensionPixelSize(R.styleable.CornerImageView_riv_border_width, -1);
        if (mBorderWidth < 0) {
            mBorderWidth = DEFAULT_BORDER_WIDTH;
        }

        mBorderColor = a.getColorStateList(R.styleable.CornerImageView_riv_border_color);
        if (mBorderColor == null) {
            mBorderColor = ColorStateList.valueOf(CornerDrawable.DEFAULT_BORDER_COLOR);
        }

        mMutateBackground = a.getBoolean(R.styleable.CornerImageView_riv_mutate_background, false);
        mIsOval = a.getBoolean(R.styleable.CornerImageView_riv_oval, false);

        final int tileMode = a.getInt(R.styleable.CornerImageView_riv_tile_mode, TILE_MODE_UNDEFINED);
        ...
        final int tileModeX =
                a.getInt(R.styleable.CornerImageView_riv_tile_mode_x, TILE_MODE_UNDEFINED);
        if (tileModeX != TILE_MODE_UNDEFINED) {
            setTileModeX(parseTileMode(tileModeX));
        }

        final int tileModeY =
                a.getInt(R.styleable.CornerImageView_riv_tile_mode_y, TILE_MODE_UNDEFINED);
        ...
        if (mMutateBackground) {
            ...
            // so it's false by default, what doesn't allow to create the CornerDrawable. At this point, after load
            // mMutateBackground and updated BackgroundDrawable to CornerDrawable, the View's background drawable needs to
    ...
    public void setImageDrawable(Drawable drawable) {
        mResource = 0;
        mDrawable = CornerDrawable.fromDrawable(drawable);
        updateDrawableAttrs();
    ...
    public void setImageBitmap(Bitmap bm) {
        mResource = 0;
        mDrawable = CornerDrawable.fromBitmap(bm);
        updateDrawableAttrs();
        super.setImageDrawable(mDrawable);
    }
    ...
    private Drawable resolveResource() {
        ...
        return CornerDrawable.fromDrawable(d);
    }
    ...
    private void updateBackgroundDrawableAttrs(boolean convert) {
        if (mMutateBackground) {
            if (convert) {
                mBackgroundDrawable = CornerDrawable.fromDrawable(mBackgroundDrawable);
            }
    ...
    private void updateAttrs(Drawable drawable, ScaleType scaleType) {
        ...
        if (drawable instanceof CornerDrawable) {
            ...
            ((CornerDrawable) drawable).setCornerRadius(
                    mCornerRadii[Corner.TOP_LEFT],
                    mCornerRadii[Corner.TOP_RIGHT],
                    mCornerRadii[Corner.BOTTOM_RIGHT],
                    mCornerRadii[Corner.BOTTOM_LEFT]);
    ...
    public void setBorderColor(ColorStateList colors) {
        ...
        mBorderColor =
                (colors != null) ? colors : ColorStateList.valueOf(CornerDrawable.DEFAULT_BORDER_COLOR);
        updateDrawableAttrs();
        updateBackgroundDrawableAttrs(false);
        if (mBorderWidth > 0) {
            invalidate();
